    ...
    return function (req, resp, next) {
        ...
        var myReq = request(opts, function (myRes) {
            ...
            myRes.on('error', function (err) {
                onErrorHandler(myRes , opts , next , err);
            });
        ...
        myReq.on('error', function (err) {
            onErrorHandler(myReq , opts , next , err);
        });
        if (!req.readable) {
            onUnreadHandler(opts , req , myReq);
        } else {
            outputHandler(opts , req , myReq);
        }
    };
};
...
/**
 * this is where we can try to catch the error and service up the json file
 *
 */
const onErrorHandler = function(myReq , options , next , err)
{
    if (options.debug) {
        console.log('onErrorHandler' , options , err);
    }
    next(err);
};

const outputHandler = function(options , req , myReq)
{
    if (options.debug) {
        console.log('outputHandler' , options);
    }
    req.pipe(myReq);
};

const onUnreadHandler = function(options , req , myReq)
{
    if (options.debug) {
        console.log('onUnreadHandler' , options);
    }
    myReq.end();
};
